...
| 3    | **Generate Commit Message** | Analyzes staged changes, generates conventional commit message, shows for approval. Appends [AI attribution](#ai-attribution) to the commit. |
| 4    | **Push to Remote**          | Sets upstream if needed (`git push -u origin <branch>`).                            |
| 5    | **Create or Find PR**       | Creates PR via `gh pr create` with a [structured PR body](#structured-pr-descriptions), or finds an existing one. Respects [PR templates](#pr-template-detection) when present. |
...
If unresolved findings from `/uf.review-council` exist, `/finale` adds a **Known Issues** section listing them. This section is omitted when no review was run or all findings were resolved.

The PR body ends with an attribution footer: `This PR was generated by /uf.finale (AI-assisted).`
